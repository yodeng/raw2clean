...
import argparse,multiprocessing,gzip,bz2file,os,datetime
...
def main():
    ...
    if args.SE:
        ...
        sum_by_pos(seqlist,abspath(join(args.out_dir,os.path.splitext(os.path.basename(seqfile))[0]+".sum_pos.txt")),abspath(join(args.out_dir,os.path.splitext(os.path.basename(seqfile))[0]+".sum_avgQ.txt")),args.phread)
    elif args.PE:
        ...
        p1 = multiprocessing.Process(target=sum_by_pos,args=(r1seqlist,abspath(join(args.out_dir,os.path.splitext(os.path.basename(seqfile1))[0]+".sum_pos.R1.txt")),abspath(join(args.out_dir,os.path.splitext(os.path.basename(seqfile1))[0]+".sum_avgQ.R1.txt")),args.phread));p1.start()
        p2 = multiprocessing.Process(target=sum_by_pos,args=(r2seqlist,abspath(join(args.out_dir,os.path.splitext(os.path.basename(seqfile2))[0]+".sum_pos.R2.txt")),abspath(join(args.out_dir,os.path.splitext(os.path.basename(seqfile2))[0]+".sum_avgQ.R2.txt")),args.phread));p2.start()
        p1.join();p2.join()
    else:
